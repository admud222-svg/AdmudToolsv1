...
// IMPORT MENU SHOP CONFIG YANG BARU!
import { openShopConfig } from "./plugin/shop/shop.js";
...
export function openAdminMenu(player) {
    const form = new ActionFormData()
        ...
        .button("Member Set\n§8(Clan, Land, Shop, dll)", "textures/ui/icon_multiplayer")
...
function menuEditRank(player, rankID, isNew) {
    ...
    forceShow(player, form, res => {
        ...
        ranks[newID] = { 
            ...
            warpLimit: parseInt(res.formValues[6]) || 1, 
...
// ==========================================
// ADMIN: MEMBER, CLAN, RTP, LAND, PWARPS, SHOP
// ==========================================
function menuMemberSet(player) {
    const form = new ActionFormData()
        ...
        .button("§lPlayer Warps\n§r§8Manage & Hapus Warp Player", "textures/items/ender_pearl") 
        .button("§lToggles Menu\n§r§8Aktif/Matikan Fitur Server", "textures/ui/settings_glyph_color_2x")
        // INI DIA TOMBOL SHOP CONFIG YANG BARU DITAMBAHKAN
        .button("§lShop Config\n§r§8Atur Harga & Restock", "textures/ui/icon_recipe_item")
        .button("§cKembali", "textures/ui/cancel");

    forceShow(player, form, res => {
        if (res.canceled || res.selection === 6) return openAdminMenu(player); // JIKA KEMBALI
        ...
        // JIKA TOMBOL SHOP CONFIG DIKLIK
        if (res.selection === 5) openShopConfig(player); 
    });
}
